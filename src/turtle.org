...
* Namespace: thi.ng.sema.io.turtle

#+BEGIN_SRC clojure :tangle ../babel/src/cljx/thi/ng/sema/io/turtle.cljx :mkdirp yes :padline no
  (ns thi.ng.sema.io.turtle
    (:refer-clojure :exclude [peek])
    (:require
     [thi.ng.sema.core.api :as api :refer [default-prefixes]]
      ...
      (if iri
        (let [{ctx :iri-ctx prefix :prefix-ns} state]
          (condp = ctx
            ...
            (-> state
                (assoc ctx (api/resource (resolve-iri state iri)))
        ...
        (if id
          (let [ctx (:triple-ctx state)
                state (if (get-in state [:blanks id])
                        state
                        (assoc-in state [:blanks id] (api/blank-node)))
                state (-> state
                          (transition)
    ...
    (let [src (str (:literal state) (read-until-literal in (str (:lit-terminator state))))
      ...
      (if lit
        (condp = n
          ...
          (let [state (-> state (transition) (assoc :object (api/literal lit)))]
            ((:state state) in state)))
  ...
  (defn parse-lang-tag
    ...
    (let [ok (:lang-tag char-ranges)
      ...
      (if (re-matches #"(?i)[a-z]{2,3}(-[a-z0-9]+)*" lang)
        (let [state (-> state (transition) (assoc :object (api/literal (:literal state) lang)))]
          ((:state state) in state))
  ...
  (defn parse-literal-type
    ...
    (if (next-char? in \^)
      (let [[iri state] (if (= (peek in) \<) (read-iri-ref in state) (read-pname in state))]
        (if iri
          (let [state (-> state (transition) (assoc :object (api/literal (:literal state) nil iri)))]
            ((:state state) in state))
          state))))
